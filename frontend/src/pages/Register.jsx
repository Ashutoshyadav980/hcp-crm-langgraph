...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(authFailure(err.response?.data?.detail || 'Registration failed. Try again.'));
    }
  };
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-900 font-sans p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-800/80 border border-slate-700/50 rounded-2xl p-8 backdrop-blur-xl shadow-2xl z-10">
        ...
        <div className="text-center mb-8">
          <div className="w-12 h-12 rounded-xl gradient-bg flex items-center justify-center text-white font-bold text-2xl mx-auto mb-4 shadow-lg">
            +
          </div>
          <h2 className="text-2xl font-bold text-white">Create an account</h2>
          <p className="text-slate-400 text-sm mt-2">Get started with our AI-First Healthcare CRM</p>
        </div>

        {/* Success alert */}
        {success && (
          <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 p-3 rounded-lg text-sm mb-6">
        ...
        <div className="text-center mt-6 text-slate-400 text-sm">
          ...
          <Link to="/login" className="text-sky-400 hover:text-sky-300 font-semibold underline transition">
            Sign In here
          </Link>
        </div>
      </div>
